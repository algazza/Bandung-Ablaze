  ...
  return (
    <div className="grid md:grid-cols-3 grid-cols-1 gap-4 justify-around">
      {TokohPenting.map((tokoh, index) => (
        <div key={index} className={`flex flex-col items-center md:px-6 py-6 ${
            index === 1 ? "md:border-x-2 md:border-y-0 border-y-2 border-black" : ""
        }`}>
          <img src={tokoh.img} alt={tokoh.img} className="rounded-t-full md:w-[300px] max-h-[730px]" />
